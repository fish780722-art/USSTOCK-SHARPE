...
- 無價格或資料不足的 ticker 會自動忽略，並在資料品質區塊列出被忽略標的及其有效價格日期範圍
- 可自訂「抽取前幾名」：先從有效標的中挑出單股 Sharpe 最高的前 N 名，再做投組最佳化
- 使用高維穩定模式，以 shrinkage covariance 與 long-only tangency portfolio 近似最大 Sharpe 解
